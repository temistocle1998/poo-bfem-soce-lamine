...
def update():
    ...
    try:
       sql = "Update  candidat set empname= %s,mobile= %s,salary= %s where id= %s"
...
def show():
        mysqldb = mysql.connector.connect(host="localhost", user="phpmyadmin", password="djarafat10", database="python_poo")
        mycursor = mysqldb.cursor()
        mycursor.execute("SELECT id,prenom_s,nom,numero_table FROM candidat")
...
tk.Label(root, text="Employee candidat", fg="red", font=(None, 30)).place(x=300, y=5)
